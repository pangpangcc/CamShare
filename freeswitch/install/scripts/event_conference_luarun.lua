...
freeswitch.consoleLog("DEBUG", "# 会议事件监听脚本(启动运行脚本)->开始");
...
if fun == "conference_member_add" then
  ...
  freeswitch.consoleLog("NOTICE", "# 会议事件监听脚本(启动运行脚本)->事件:channel_id:" .. channel_id .. "\n");
...
--  api:execute("conference", conference_name .. " relate " .. member_id .. " " .. member_id .. " nohear");
  ...
  if member_type == "moderator" then
    freeswitch.consoleLog("NOTICE", "# 会议事件监听脚本(启动运行脚本)->事件:增加主持人[member_id:" .. member_id .. "]到会议室[" .. conference_name .. "]\n");   
...
--    api:execute("conference", conference_name .. " record /usr/local/freeswitch/recordings/video_h264/" .. conference_name .. os.date("_%Y%m%d%H%M%S", os.time()) .. ".h264");
    ...
    freeswitch.consoleLog("NOTICE", "# 会议事件监听脚本(启动运行脚本)->事件:增加成员[member_id:" .. member_id .. "]到会议室[" .. conference_name .. "]\n");
  end
  
end -- 处理会议增加成员完成

freeswitch.consoleLog("DEBUG", "# 会议事件监听脚本(启动运行脚本)->结束\n");
