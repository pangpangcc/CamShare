...
if fun == "conference_member_add" then
  ...
  freeswitch.consoleLog("CONSOLE", "# 会议事件监听脚本->事件:channel_id:" .. channel_id .. "\n");
...
--    api:execute("uuid_setvar", channel_id .. " enable_file_write_buffering false");
    api:execute("conference", conference_name .. " record /usr/local/freeswitch/recordings/" .. conference_name .. os.date("_%Y%m%d%H%M%S", os.time()) .. ".h264");
  else
