										...
										const char *userdata, switch_log_level_t level, const char *fmt, va_list ap)
{
	...
	uint32_t len;
	// Modify by Max 2017/01/23
	switch_thread_id_t pid = switch_thread_self();
	...
	if (channel != SWITCH_CHANNEL_ID_LOG_CLEAN) {
		...
		switch_snprintf(pids, sizeof(pids), "%lld", pid);
...
//		len = (uint32_t) (strlen(extra_fmt) + strlen(date) + strlen(filep) + 32 + strlen(fmt));
		len = (uint32_t) (strlen(extra_fmt) + strlen(date) + strlen(pids) + strlen(filep) + 32 + strlen(fmt));
#endif
...
//		switch_snprintf(new_fmt, len, extra_fmt, date, switch_log_level2str(level), filep, line, 128, fmt);
		switch_snprintf(new_fmt, len, extra_fmt, date, switch_log_level2str(level), pids, filep, line, 128, fmt);
#endif
