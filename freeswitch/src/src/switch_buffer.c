...
SWITCH_DECLARE(switch_size_t) switch_buffer_recorvery(switch_buffer_t *buffer, switch_size_t datalen)
{
	switch_size_t last = 0;

	if (buffer->used < 1) {
		buffer->used = 0;
		return 0;
	} else if (buffer->used >= datalen) {
		last = buffer->used - datalen;
	} else {
		return 0;
	}

	memcpy(buffer->data, buffer->head + datalen, last);
	buffer->used = buffer->actually_used = last;
	buffer->head = buffer->data;
	buffer->head[last] = '\0';
...
SWITCH_DECLARE(void) switch_buffer_bzero(switch_buffer_t *buffer) {
	switch_buffer_zero(buffer);

	memset(buffer->head + buffer->used, '\0', buffer->datalen - buffer->used);
