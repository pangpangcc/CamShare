...
static void *SWITCH_THREAD_FUNC switch_event_dispatch_thread(switch_thread_t *thread, void *obj)
{
	...
	for (;;) {
		...
		// Modify for Max 201701/24
		if( switch_queue_size(queue) >= (DISPATCH_QUEUE_LEN * DISPATCH_THREAD_COUNT) - 1 ) {
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING, "Pop DISPATCH_QUEUE Full\n");
		}
...
static switch_status_t switch_event_queue_dispatch_event(switch_event_t **eventp)
{
	...
	while (event) {
		...
		*eventp = NULL;
		...
		// Modify for Max 201701/24
		switch_mutex_lock(EVENT_QUEUE_MUTEX);
		if( switch_queue_size(EVENT_DISPATCH_QUEUE) >= (DISPATCH_QUEUE_LEN * DISPATCH_THREAD_COUNT) - 1 ) {
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING, "Push DISPATCH_QUEUE Full\n");
			switch_yield(switch_time_make(0, 100000));
		}
		switch_mutex_unlock(EVENT_QUEUE_MUTEX);
