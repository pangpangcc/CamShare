...
bool TcpServer::Start(switch_memory_pool_t *pool, const char *ip, switch_port_t port) {
	...
	if(switch_socket_create_pollfd(&mpPollfd, mpSocket->socket, SWITCH_POLLIN, mpSocket, pool) != SWITCH_STATUS_SUCCESS ) {
...
void TcpServer::Stop() {
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "TcpServer::Stop() \n");
	...
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "TcpServer::Stop( [Success] ) \n");
}
...
void TcpServer::Disconnect(Socket* socket) {
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "TcpServer::Disconnect( socket : %p, %s:%d ) \n", socket, socket->ip, socket->port);
...
void TcpServer::Close(Socket* socket) {
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "TcpServer::Close( socket : %p, %s:%d ) \n", socket, socket->ip, socket->port);
...
//			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "TcpServer::IOHandleThread( [pollset_poll intr], this : %p ) \n", this);
			continue;
		} else if (status == SWITCH_STATUS_TIMEOUT) {
//			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "TcpServer::IOHandleThread( [pollset_poll timeout], this : %p ) \n", this);
			// sleep
			switch_cond_next();
		} else if (status != SWITCH_STATUS_SUCCESS) {
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "TcpServer::IOHandleThread( [pollset_poll failed, status : %d] ) \n", status);
		}
...
//					switch_mutex_unlock(mpSocketMutex);
					...
					if (switch_socket_accept(&newsocket, mpSocket->socket, socket->pool) != SWITCH_STATUS_SUCCESS) {
						...
						if( mpTcpServerCallback && mpTcpServerCallback->OnAccept(socket) ) {
							...
							// 开始监听接收事件
							socket->CreatePollfd(SWITCH_POLLIN | SWITCH_POLLLET);
...
//				}
				...
				switch_log_printf(
						SWITCH_CHANNEL_LOG,
						SWITCH_LOG_DEBUG,
						"TcpServer::IOHandleThread( [Read Event], socket : %p, rtnevents : 0x%x )\n",
						socket, fds[i].rtnevents
						);
