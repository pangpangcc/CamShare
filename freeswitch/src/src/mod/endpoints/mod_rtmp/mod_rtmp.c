...
//	switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "rtmp set channel variables\n");
...
switch_status_t rtmp_tech_init(rtmp_private_t *tech_pvt, rtmp_session_t *rsession, switch_core_session_t *session)
{
	...
	// add by samson for test
//	switch_log_printf(SWITCH_CHANNEL_UUID_LOG(rsession->uuid), SWITCH_LOG_DEBUG, "rtmp tech init\n");
...
switch_status_t rtmp_kill_channel(switch_core_session_t *session, int sig)
{
	...
	// add by samson for test
//	switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "rtmp kill channel, sig:%d\n", sig);
...
switch_status_t rtmp_read_frame(switch_core_session_t *session, switch_frame_t **frame, switch_io_flag_t flags, int stream_id)
{
	...
	if (rsession->state >= RS_DESTROY) {
		// add by samson for test
//		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG
//			, "rtmp read frame, state==RS_DESTROY.\n");
		return SWITCH_STATUS_FALSE;
	}
...
switch_status_t rtmp_session_request(rtmp_profile_t *profile, rtmp_session_t **newsession)
{
	...
	*newsession = switch_core_alloc(pool, sizeof(rtmp_session_t));
	...
	// create handle param
	switch_mutex_init(&((*newsession)->handle_mutex), SWITCH_MUTEX_NESTED, pool);
	// create handle count param
	(*newsession)->handle_count = 0;
	(*newsession)->tryread_times = 0;
...
switch_status_t rtmp_real_session_destroy(rtmp_session_t **rsession)
{
	...
	if (NULL == (*rsession)->account || NULL == (*rsession)->account->user) {
		...
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, "RTMP session ended [%s], [%s]\n", (*rsession)->uuid, (*rsession)->account->user);
	}
...
switch_call_cause_t rtmp_session_create_call(rtmp_session_t *rsession, switch_core_session_t **newsession, int read_channel, int write_channel, const char *number, const char *auth_user, const char *auth_domain, switch_event_t *event)
{
	...
	if (NULL == rsession->account || NULL == rsession->account->user) {
		...
		switch_log_printf(SWITCH_CHANNEL_UUID_LOG(rsession->uuid), SWITCH_LOG_INFO, "New FreeSWITCH session created: %s [%s]\n",
				rsession->account->user, switch_core_session_get_uuid(*newsession));
	}
...
SWITCH_MODULE_SHUTDOWN_FUNCTION(mod_rtmp_shutdown)
{
	...
	// add by samson for test
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, "rtmp shutdown\n");
...
//		switch_yield(10000000);
	...
	// add by samson for test
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, "rtmp runtime end\n");
