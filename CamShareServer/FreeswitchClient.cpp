...
bool FreeswitchClient::StartUserRecvVideo(
	...
	LogManager::GetLogManager()->Log(
			...
			"event : [Freeswitch-允许用户开始观看聊天室视频], "
			"user : %s, "
			"conference : %s, "
			"type : %d "
			")",
		...
		if( memberId.length() > 0 ) {
			...
			if( SendCommandGetResult(temp, result) ) {
				LogManager::GetLogManager()->Log(
						LOG_MSG,
						"FreeswitchClient::StartUserRecvVideo( "
						"event : [Freeswitch-允许用户开始观看聊天室视频-还原权限], "
						"user : %s, "
						"conference : %s, "
						"type : %d "
						")",
			...
			if( bFlag ) {
				...
				case Member:{
					LogManager::GetLogManager()->Log(
							LOG_WARNING,
							"FreeswitchClient::StartUserRecvVideo( "
							"event : [Freeswitch-允许用户开始观看聊天室视频-开放普通成员权限], "
							"user : %s, "
							"conference : %s, "
							"type : %d "
							")",
				...
				case Moderator:{
					...
					if( SendCommandGetResult(temp, result) ) {
						LogManager::GetLogManager()->Log(
								LOG_WARNING,
								"FreeswitchClient::StartUserRecvVideo( "
								"event : [Freeswitch-允许用户开始观看聊天室视频-开放主持人权限], "
								"user : %s, "
								"conference : %s, "
								"type : %d "
								")",
	...
	if( !bFlag ) {
		LogManager::GetLogManager()->Log(
				LOG_ERR_USER,
				"FreeswitchClient::StartUserRecvVideo( "
				"event : [Freeswitch-允许用户开始观看聊天室视频-失败], "
				"user : %s, "
				"conference : %s, "
				"type : %d "
				")",
...
list<string> FreeswitchClient::GetMemberIdByUserFromConference(
	...
	LogManager::GetLogManager()->Log(
			...
			"event : [Freeswitch-在聊天室获取用户member_id], "
			"user : %s, "
			"conference : %s "
			")",
			user.c_str(),
	...
	if( SendCommandGetResult(temp, result) ) {
		LogManager::GetLogManager()->Log(
				LOG_STAT,
				"FreeswitchClient::GetMemberIdByUserFromConference( "
				"event : [Freeswitch-在聊天室获取用户member_id-获取会议会员列表-成功] "
				")"
				);
					...
					if( itr != mRtmpChannel2UserMap.End() ) {
						...
						if( channel->user == user && channel->conference == conference ) {
							LogManager::GetLogManager()->Log(
									LOG_STAT,
									"FreeswitchClient::GetMemberIdByUserFromConference( "
									"event : [Freeswitch-在聊天室获取用户member_id-找到对应channel], "
									"user : %s, "
									"conference : %s, "
									"channelId : %s "
									")",
	...
	if( !bFlag ) {
		LogManager::GetLogManager()->Log(
				LOG_STAT,
				"FreeswitchClient::GetMemberIdByUserFromConference( "
				"event : [Freeswitch-在聊天室获取用户member_id-失败], "
				"user : %s, "
				"conference : %s "
				")",
				user.c_str(),
