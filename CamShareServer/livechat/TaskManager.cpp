...
// 初始化接口
bool CTaskManager::Init(const list<string>& svrIPs, unsigned int svrPort, ILiveChatClient* client, ILiveChatClientListener* clientListener, ITaskManagerListener* mgrListener)
{
...
void CTaskManager::OnRecv(const TransportProtocol* tp)
{
	...
	if (IsRequestCmd(tp->header.cmd)) {
		...
		if (NULL != task) {
			...
			task->Init(m_client, m_clientListener);
		}
	}
