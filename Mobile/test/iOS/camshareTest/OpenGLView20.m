...
//    }
    @synchronized(self)
    {
        ...
        char* p = (char*)data;
        glBindTexture(GL_TEXTURE_2D, _textureYUV[TEXU]);
        glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, w/2, h/2, GL_RED_EXT, GL_UNSIGNED_BYTE, (const GLvoid*)(p + w * h));
        
       // [self debugGlError];
        
        glBindTexture(GL_TEXTURE_2D, _textureYUV[TEXV]);
        glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, w/2, h/2, GL_RED_EXT, GL_UNSIGNED_BYTE, (const GLvoid*)(p + w * h * 5 / 4));
...
- (void)setVideoSize:(GLuint)width height:(GLuint)height
{
    ...
    char *blackData =(char *) malloc(width * height * 1.5);
	if(blackData)
