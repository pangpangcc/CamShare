...
CamshareClient::CamshareClient() {
    ...
    mEncoderFormat = VIDEO_FORMATE_NV21;
}
...
// 编码和发送视频数据
bool CamshareClient::InsertVideoData(unsigned char* data, unsigned int len, unsigned long timestamp, int width, int heigh, Direction direction) {
	...
	if (mCaptureVideoStart && data && len > 1)
	{
		...
		if(mVideoData)
		{
            ...
            switch (mEncoderFormat) {
                case camshare::VIDEO_FORMATE_NV12:
                {
                    yuvData = data;
                    for (int i = 0; i < width * heigh/2; i+=2) {
                        unsigned char videoData = data[width * heigh + i];
                        yuvData[width * heigh + i] = data[width * heigh + i +1];
                        yuvData[width * heigh + i +1] = videoData;
                    }
                }
                    break;
                case camshare::VIDEO_FORMATE_NV21:
                {
                    yuvData = data;
                }
            ...
            if (yuvData) {
                CaptureBuffer* item = new CaptureBuffer(yuvData, len, timestamp, width, heigh, direction);
...
// 选择采集视频格式
int CamshareClient::ChooseVideoFormate(int* videoFormate, int size, DeviceType deviceType)
{
	int result = -1;
	if (mVideoData){
    ...
    if (result > 0) {
        mEncoderFormat = (VIDEO_FORMATE_TYPE)result;
    }
	return result;
}
...
/**
 设置解码视频格式
 */
bool CamshareClient::SetDecoderVideoFormate(VIDEO_FORMATE_TYPE type)
{
    bool result = false;

    result = mH264Decoder.SetDecoderVideoFormat(type);
    return result;
}

string CamshareClient::GetUser() {
    return user;
}

string CamshareClient::GetDest() {
    return dest;
}
...
void CamshareClient::OnMakeCall(RtmpClient* client, bool success, const string& channelId) {
	...
	if( success ) {
		if( client->GetUser() == this->dest ) {			// 进入自己会议室, 上传
			...
			snprintf(filePath, sizeof(filePath), "%s", mFilePath.c_str());
            mH264Decoder.SetRecordFile(filePath);
		...
		if (mCaptureVideoStart && !mVideoData->IsVideoDataEmpty())
		{
			...
			else
			{
				...
				if(NULL != item){
					...
					if (mpCamshareClientListener != NULL)
					{
						 mpCamshareClientListener->OnCallVideo(this, item->data, item->width, item->heigth, (int)timeInterval);
					}
					item->timeInterval = (int)timeInterval;
...
// 开始心跳包
void CamshareClient::StartHeartBeat(){
	// 开始心跳线程
...
// 停止心跳包
void CamshareClient::StopHeartBeat(){
