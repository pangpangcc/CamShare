...
static unsigned char* detailPic0(unsigned char* data, int imageWidth, int imageHeight, int newImageW, int newImageH) {
    ...
    if (NULL != data
        ...
        && (newImageH > 0))
    {
        yuv_temp =new unsigned char[newImageW*newImageH*3/2];
...
//旋转270度，裁剪，NV21转YUV420P（竖着录制前摄像头）
...
//
	...
	mIsPush = false;
    formateType = VIDEO_FORMATE_NONE;
}
...
// 选择视频采集格式
int CaptureData::ChooseVideoFormate(int* videoFormate, int size, DeviceType deviceType)
{
	int result = -1;
    if( videoFormate ) {
            for (int i = 0; i < size; i++){
                if (videoFormate[i] > VIDEO_FORMATE_NONE && ((videoFormate[i] < result) || (result < 0))){
